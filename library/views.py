...
def studentsbooks(request):
    if request.method == "POST":
        ...
        book = Book(title=title, author=author, rack_no=rack_no, description=description, available=available)
        book.save()
        if book.categories.add(cat):
            return redirect('/books')
    books = Book.objects.all()
    categories = Category.objects.all()
    return render(request, "studentsbook.html", {"books": books, "categories": categories})
...
def delete_student(request, id):
    category = Student.objects.filter(id=id)
    category.delete()
